...
- **Deleted-anchor-row detection is partial:** `VerifyAnchors` now enforces contiguous anchor coverage from seq 1, so deleting a leading/interior anchor row surfaces as "anchor gap". It cannot distinguish "all anchors for a chain deleted" or "tail simply not yet anchored" from each other — both look like a clean, shorter `AnchoredThrough`. Closing that gap needs diffing against the external sink/export (Phase 4).
